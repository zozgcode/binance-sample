...
export default function Losers() {
  ...
  const losersData = data.filter(({ percent_change_24h }) => parseFloat(percent_change_24h) < 0);

  return (
    <div>
      {losersData.map(({ id, symbol, price_usd, percent_change_24h }) => (
        <div
          key={id}
        ...
        >
          ...
          <div className="flex items-center gap-16">
            ...
            <button
              className={`h-[35px] w-[90px] leading-[24px] rounded-[5px] px-4 text-[12px] bg-red-500`}>
              {percent_change_24h}
            </button>
          </div>
        </div>
